...
const handleCateogry = async () => {
  ...
  data.data.forEach((ph) => {
    ...
    div.innerHTML = `
        <button onclick="videosLoading('${ph.category_id}')" class="btn focus:bg-red-500" >${ph.category}</button>
        `;
...
const videosLoading = async (catID) => {
  const res = await fetch(
  ...
  data.data.forEach((videos) => {
    ...
    const dataInteger = parseInt(date);

    const hours = Math.floor(dataInteger / 3600);
    let rem = dataInteger % 3600;
      ...
      <div class="flex flex-col gap-4">
        <div class="relative">
          ...
          <div class="absolute right-0 bottom-1 rounded-lg bg-black text-white">   
            <p id="postedDate">${timeString.length > 0 ? timeString + ' ago' : ''}</p>
          </div>
        </div>
